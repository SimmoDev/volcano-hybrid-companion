...
Neither example declares an `ota:` block, so every reflash is over USB — there is no over-the-air update path. That is a deliberate omission to keep the examples minimal; a Dial mounted somewhere awkward to reach would want `ota:` (with `safe_mode`) added.
