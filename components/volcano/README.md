...
A command is a request, never an immediate state change (ADR-0002): `set_heater(true)` does not make `device.heater().value()` become `true`. Each field resolves through whichever confirmation source the protocol actually gives it — heater, pump, vibration, display-on-cooling and display-units resolve from the device's own notification; target temperature, LED brightness and auto-shutoff duration have no such notification (STATE-013), so each is confirmed by an explicit read-back issued right after the write. `requested()` always clears once the device answers with a matching value, whatever the source. A *differing* answer only clears it early where the report is deterministically attributable to that specific write — a read-back, for the three read-back-confirmed fields — landing `value()` on whatever the device actually reported (STATE-013's silent-drop case). A differing *notification* never clears it early on its own, for any field: a notification carries no request identity and can be triggered by a bit unrelated to the field being resolved (e.g. a vibration-alert pulse sharing the status register with the pump bit), so treating one as a drop signal would be a false positive rather than STATE-013's evidenced behaviour, which has only ever been observed on target temperature. Every field still resolves eventually either way, via a 5-second timeout with no answer at all, in which case `value()` is left untouched — see [ADR-0009](../../docs/decisions/ADR-0009-volcano-abstraction-layer-interface.md)'s Notes for the full reasoning.

`set_auto_shutoff_duration()` and `set_led_brightness()` are additionally refused (logged, nothing written) for the brief window between a connection starting and `connection_state()` reaching `READY`: both characteristics are also read once per connection as part of the initial state sweep, and a write's own read-back reads the same characteristic a second time — `ESP_GATTC_READ_CHAR_EVT` carries no request identity, so the sweep's own read and the read-back would be indistinguishable. The other six commands have no such collision to avoid: their characteristics are not static-read-queue members, so their handles are already resolved and usable as soon as service discovery completes, seconds before `READY`. Issuing one that early is still safe — the requested-versus-confirmed handling above only trusts a report as a given write's own outcome once that write's ack has arrived, so it cannot be confused with an earlier, unrelated report on the same characteristic. `READY` is still the point a connection has anything meaningful to show a control interface, which is reason enough to treat it as when commands become worth issuing — just not a restriction the other six enforce themselves.
...
Every value `VolcanoDevice` tracks can be exposed as a standard ESPHome entity, configured directly under the `volcano:` block — see "Configuration" below. Read-only values get a sensor or text_sensor; each writable one gets a single two-way entity that both reports the device's current value and sets a new one — a number for the target temperature, the auto-shutoff duration and LED brightness, and a switch for the heater, pump, vibration, display-on-cooling and display-units. `VolcanoComponent` is what wires these: an entity's `control()`/`write_state()` forwards into the matching `VolcanoDevice::set_*()`, and `VolcanoDevice`'s state-change callback publishes to whichever entities are configured, whenever a field they carry actually changes. Once configured, an entity is reachable from anywhere ESPHome surfaces entities — a `web_server` page, or Home Assistant when the `api` component is enabled ([ADR-0012](../../docs/decisions/ADR-0012-home-assistant-integration.md)); nothing in this component is conditional on either being present.
