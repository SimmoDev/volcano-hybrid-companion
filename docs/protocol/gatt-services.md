...
- **Implementation status**: Connecting by address is handled by the parent `ble_client`/`esp32_ble_tracker` rather than by the connection path, per [ADR-0007](../decisions/ADR-0007-ble-connection-lifecycle.md). Finding the address in the first place is `VolcanoPairing`'s ([ADR-0013](../decisions/ADR-0013-release-and-distribution.md)): it extracts the serial from the manufacturer data and matches the local name, seen as separate events under the same address, and reads nothing of the undecoded 14-byte remainder..
...
- **Interpretation**: The connection is a control channel, not a dead-man's handle. A client that crashes, loses power, or is force-quit leaves the heater and pump running, and the only thing that will stop them is the auto-shutoff countdown ([STATE-005](state-model.md#state-005--auto-shutoff-countdown)). How long that leaves the device unattended depends on the configured duration: the official app will only set 30 minutes at the shortest, but the device itself accepts far less — a 60-second duration has been confirmed accepted and honoured through to an actual expiry ([CMD-003](commands.md#cmd-003--set-auto-shutoff-duration)) — so a controller can configure a substantially tighter backstop than the app allows. Any controller must be designed on the basis that its own failure does not make the device safe.
