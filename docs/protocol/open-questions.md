...
Most of what remains needs this project's own BLE client rather than further observation of the official app. That app exercises a fixed set of operations, clamps its own inputs, subscribes to 8 of the device's 16 notify-capable characteristics, and never writes most of the writable ones — so any question that turns on writing an unexercised characteristic, or on issuing a value outside the range the app will send, is beyond what observing it can answer. Those were gated on the Volcano component reaching the point where it can drive the device itself, which it now has: it subscribes to every notify-capable characteristic and issues every write recorded in [`commands.md`](commands.md), so the questions below that turn on subscribing or on issuing a specific value are answerable rather than blocked. A few of the entries below need neither: they turn on device behaviour that can be observed with the app, a second host, or simply time.
