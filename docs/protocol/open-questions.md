...
See [CHAR-011](characteristics.md#char-011--unknown-value-hist1) and [CHAR-012](characteristics.md#char-012--unknown-value-hist2). `0x0042` reads as ASCII `"616161…"`; `0x0044` reads as ASCII all-zero-character text (not raw null bytes). A third-party source (unverified) names these HIST1 and HIST2 respectively; no other third-party source mentions either. What either value means is unknown, and neither moves. Read repeatedly across pump activations, heating cycles, target changes, reconnections and a change to each of the two settings the app exposes, `0x0042`'s 16 bytes were identical every time, and `0x0044` never left its all-zero state. Nothing a user can do through the app moves either one. Neither value corresponds to anything the app displays. The official client reads both as part of its Analysis function and, when a fault is present, renders them into a plain-text block alongside the serial number and a timestamp for the user to send to Storz & Bickel support — so they appear to be service/diagnostic history intended for the manufacturer rather than anything a user-facing client interprets. That is taken from the client only and has not been independently verified, and it does not establish what the values encode. Would be resolved by reading both either side of the actions not yet tried — a full session ending in auto-shutoff expiry, or simply a period of days of ordinary use — and by reading them on a second unit. A value that stays byte-identical across every action a user can perform is more likely a fixed per-device characteristic than a counter, and confirming that would change what this question is asking.
...
## Auto-shutoff duration — whether the device accepts less than 30 minutes

See [CMD-003](commands.md#cmd-003--set-auto-shutoff-duration). Writes at both ends of the app's 30-to-360-minute range have been captured, but that range is the app's UI constraint; what the device itself accepts has not been tested. This is the most useful unknown in this file: the auto-shutoff countdown is the only known backstop against a controller dying with the heater on, and the app's 30-minute floor is what makes that backstop weak (see [How long a client can be absent before the device intervenes](#how-long-a-client-can-be-absent-before-the-device-intervenes)). If the device accepts a shorter duration, a controller can narrow its own worst case substantially at no cost. Would be resolved by writing durations below `1800` and reading the value back, then confirming against an actual expiry that the shorter duration is honoured rather than silently clamped.
...
- Whether a directed connection to the device's known address behaves differently from a scan-then-connect. The device accepts only one connection at a time and stops advertising while connected ([CONN-003](gatt-services.md#conn-003--single-connection-at-a-time)), but the device has only ever been connected to by scanning first. A controller reconnecting after a dropout would instead connect directly to a remembered address, and whether that fails cleanly, hangs, or succeeds where a scan-then-connect could not is untested. This is not reachable with the official app, whose connect action always opens a chooser and scans; it needs a client that can connect to a remembered address directly. Would be resolved by issuing a direct connection to the known address while a second host holds the link, and recording whether an error is returned or the attempt simply times out.
