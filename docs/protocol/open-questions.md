...
Most of what remains needs a BLE client of this project's own rather than further observation of the official app. That app exercises a fixed set of operations, clamps its own inputs, subscribes to 8 of the device's 16 notify-capable characteristics, and never writes most of the writable ones — so any question that turns on writing an unexercised characteristic, or on issuing a value outside the range the app will send, is beyond what observing it can answer. Those were gated on this project having a client capable of driving the device itself, which it now has: exploratory tooling built for this project has subscribed to every notify-capable characteristic to gather the findings below, and the shipped Volcano component (`components/volcano/volcano_ble_client.cpp`) issues every write recorded in [`commands.md`](commands.md) — so the questions below that turn on subscribing or on issuing a specific value are answerable rather than blocked. Note the shipped component itself subscribes to only 8 of the 16 (the same set the app does, minus SVC-003's characteristic, plus the vibration setting — see [CONN-001](gatt-services.md#conn-001--connection-security-and-subscription-procedure)'s Implementation status); the wider subscription coverage below came from investigation, not from what ships. A few of the entries below need neither: they turn on device behaviour that can be observed with the app, a second host, or simply time.
...
## Vendor services `00000001` and `01000002` — contents largely unexplored

See SVC-003 and SVC-004 in [`gatt-services.md`](gatt-services.md#svc-003--vendor-service-a). SVC-003's read+notify characteristic (`00000003-1989-0108-1234-123456789abc`) has been read: single byte, value `0xAE`, meaning unknown. It has also now been subscribed through a full heating cycle, pump runs and settings changes without notifying once, holding `0xAE` throughout — notable because the official app does subscribe to it, so whatever it is for, it did not move under anything exercised here. Its write-only characteristic (`00000002`) has never been written to. SVC-004's single characteristic (`01000001-1989-0108-1234-123456789abc`) has no Read property at all, so it can't be explored this way — it has never been written to or notified on. Would be resolved by writing exploratory values to the write-capable characteristics and observing any response/notification, or by capturing app traffic that exercises them.
...
Four standalone characteristics, none of them described by any third-party source. Two are stable; two move, as noted. All four have now been subscribed as well as read, so their notification behaviour is observed rather than inferred from sparse reads:
...
| `1011000e` | `0x0059` | N R | `0x00B4` (180) while the heater is on, `0x0000` while it is off | Tracks heater state exactly, changing within about a second of the status register in both directions. Reproduced across several heater transitions. Not derived from the auto-shutoff duration: the value stayed `180` with the duration set to both 1800s and 600s. What `180` itself denotes is unknown. Sits between Auto-shutoff duration (CHAR-017) and the trigger-characteristic cluster below |
| `10110017` | `0x006e` | N R | A slowly-varying value, observed between `0x014B` (331) on a cold device and `0x01D0` (464) on a thoroughly heat-soaked one, dithering by one between adjacent integers throughout | Last characteristic in the service, immediately after Minutes of Operation (CHAR-023). Characterised in full below |

`10110017` is not a counter, though sparse reads make it look like one. Subscribed, it moves in both directions — roughly as many downward steps as upward, dithering by one between adjacent integers — and traces a single rise-and-decay across a heating cycle rather than accumulating.

Its behaviour is that of a thermal mass heated indirectly, not of the regulated air:

- **It rises while the air temperature is flat at its target**, accumulating tens of counts with the controlled variable unchanging. It is therefore not a function of the value on `0x0047`.
- **It rises faster the higher the target**, with the air flat in both cases: about 1.8 counts per minute at a 210 °C target against about 3.0 at 230 °C.
- **It continues rising after the heater stops**, peaking several minutes later, by which point the air temperature has already fallen by tens of degrees.
- **It decays far more slowly than the air cools** — a handful of counts while the air fell from 165 °C to 109 °C.
- **It is unrelated to any particular air temperature**: 456 has been read at both 210 °C and 115 °C.

Its range spans at least 331 to 464: 331 on a device left unpowered overnight, and 456–464 on one thoroughly heat-soaked. Those are bounds rather than limits — a heat-soaked device exceeds what a single cycle from cold reaches even at a higher target, so the value keeps climbing over periods longer than one heating cycle.

Interpreted as tenths of a degree the observed values span roughly 33 °C to 46 °C, which is plausible for a temperature elsewhere in the device — enclosure, board or heater block. That unit is inferred from the endpoints alone and nothing establishes it. What is being measured, and in what unit, remains unidentified.

For these four: `10110017` would be resolved by identifying what physically sits where its curve implies, or by finding it in a vendor source, the behavioural characterisation above being about as far as observation alone reaches. Its practical consequence is already settled either way, in that it reports something the device does not act on and a controller has no use for. `10110004` and `10110002` need exercising through heating, pumping and auto-shutoff while watching whether anything moves them at all; neither has been observed to change. The official app subscribes to none of these four, so nothing about their notification behaviour was observable before exploratory tooling built for this project subscribed to them — the shipped Volcano component does not subscribe to any of these four either.
...
`101300ff`, the one Notify-capable entry, has now been subscribed through a full heating cycle, pump runs and settings changes without notifying once, holding its 20 zero bytes throughout — so nothing exercised so far moves it. What remains would be resolved by issuing the read-blob check its table row proposes, and correlating the rest against app UI fields not yet mapped to a handle or against third-party documentation of this service.
