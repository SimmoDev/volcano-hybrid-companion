...
| `10110017` | `0x006e` | N R | A slowly-varying value spanning `0x01C8`–`0x01D0` (456–464) across a full heating and cooling cycle, dithering by one between adjacent integers throughout | Last characteristic in the service, immediately after Minutes of Operation (CHAR-023). See below — earlier descriptions of it as a counter were mistaken |

`10110017` is not a counter, despite reading like one when sampled sparsely. Subscribed, it moves in both directions — roughly as many downward steps as upward — and traces a single rise-and-decay over a heating cycle rather than accumulating. Its behaviour across one such cycle:

- It rose from 456 to 462 while the heater was on and the **air temperature was held flat at its 210 °C target**, so it is not a function of the value on `0x0047`.
- It went on rising for about four minutes *after* the heater was switched off, peaking at 464 while the air temperature was already falling.
- It then decayed steadily as the device cooled, reaching 456 again at 115 °C.

It therefore reads 456 at both 210 °C and 115 °C air temperature, which rules out any direct relationship with the air path. Continuing to rise after the heat source is removed, then decaying slowly, is the signature of a lagging thermal mass rather than of the regulated air. A temperature elsewhere in the device — enclosure, board or heater block — would fit, but the span is narrow enough (nine counts across a full cycle) that the unit is not evident, and nothing yet identifies what is being measured.

For these four, would be resolved by continuing to correlate `10110017` against a full cold start: its value at ambient, and the excursion from cold to a heating cycle, would establish the span and may make the unit apparent. `10110004` and `10110002` need exercising through heating, pumping and auto-shutoff while watching whether anything moves them at all; neither has been observed to change. The official app subscribes to none of these four, so nothing about their notification behaviour was observable before this project's own client subscribed to them.
