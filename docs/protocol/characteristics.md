...
- **Confidence**: Confirmed
- **Implementation status**: Implemented (read-only) in `components/volcano/volcano_ble_client.cpp`, read once per connection. Verified against real hardware, returning the value recorded above.
...
- **Confidence**: Confirmed
- **Implementation status**: Implemented (read-only) in `components/volcano/volcano_ble_client.cpp`, read once per connection. Verified against real hardware. The characteristic is never written: doing so would replace a real unit's serial number.
...
- **Implementation status**: Implemented (read-only) in `components/volcano/volcano_ble_client.cpp` — resolved by UUID, subscribed, initial value read explicitly, decoded on both the initial read and subsequent notifications. Verified against real hardware, including panel-originated heater/pump toggles.
...
- **Implementation status**: Implemented for two of its bits in `components/volcano/volcano_ble_client.cpp` — subscribed, initial value read explicitly, with bit 12 decoded and written per [CMD-005](commands.md#cmd-005--set-display-on-cooling) and bit 9 decoded and written per [CMD-010](commands.md#cmd-010--set-display-units). Note the two carry opposite polarities. Verified against real hardware. The temperature-step pulse ([STATE-009](state-model.md#state-009--temperature-step-pulse-on-displayunits-register)) and the register's remaining bits are not decoded. Because the pulse makes this register notify on every 1 °C change of current temperature, the implementation logs only actual changes to the setting rather than every notification.
...
- **Implementation status**: Implemented in `components/volcano/volcano_ble_client.cpp` — subscribed, initial value read explicitly, and bit 10 decoded with its inverted polarity, alongside the write ([CMD-004](commands.md#cmd-004--set-vibration)). Verified against real hardware, including that the decoded state matches the device's actual vibration behaviour. The register's other bits are not decoded.
...
- **Confidence**: Confirmed
- **Implementation status**: Implemented (read-only) in `components/volcano/volcano_ble_client.cpp`. Verified against real hardware.
...
- **Confidence**: Confirmed
- **Implementation status**: Implemented, including the write side (CMD-001), in `components/volcano/volcano_ble_client.cpp`. Verified against real hardware.
...
- **Confidence**: Confirmed
- **Implementation status**: Implemented in `components/volcano/volcano_ble_client.cpp`: the write (CMD-002), the read-back that confirms it, and a read on each connection. The last matters because this characteristic has no notify — without it the configured brightness is unknown until something writes one. Verified against real hardware.
...
- **Confidence**: Confirmed
- **Implementation status**: Implemented (read-only) in `components/volcano/volcano_ble_client.cpp` — resolved by UUID, subscribed, initial value read explicitly, decoded on both the initial read and subsequent notifications. Verified against real hardware.
...
- **Confidence**: Confirmed
- **Implementation status**: Implemented in `components/volcano/volcano_ble_client.cpp`: the write (CMD-003), the read-back that confirms it, and a read on each connection. The last matters because this characteristic has no notify — without it the configured duration is unknown until something writes one. Verified against real hardware.
...
- **Confidence**: Confirmed
- **Implementation status**: Implemented in `components/volcano/volcano_ble_client.cpp`. Verified against real hardware.
...
- **Confidence**: Confirmed
- **Implementation status**: Implemented (read-only) in `components/volcano/volcano_ble_client.cpp` — subscribed, initial value read explicitly, decoded on both. Verified against real hardware, including the carry: minutes wrapping from 59 to 0 was seen to increment this counter.
...
- **Confidence**: Confirmed
- **Implementation status**: Implemented (read-only) in `components/volcano/volcano_ble_client.cpp` — subscribed, initial value read explicitly, decoded on both. Verified against real hardware, including a live tick while the heater was on.
...
- **Implementation status**: Implemented (read-only) in `components/volcano/volcano_ble_client.cpp`, read once per connection. Verified against real hardware, returning the value recorded above. Never written.
