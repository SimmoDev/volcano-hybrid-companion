...
- **Implementation status**: Implemented for the display-on-cooling bit only, in `components/volcano/volcano.cpp` — subscribed, initial value read explicitly, bit 12 decoded with its inverted polarity, and written per [CMD-005](commands.md#cmd-005--set-display-on-cooling). Verified against real hardware. The units bit ([STATE-010](state-model.md#state-010--temperature-display-units-bit)), the temperature-step pulse ([STATE-009](state-model.md#state-009--temperature-step-pulse-on-displayunits-register)) and the register's remaining bits are not decoded. Because the pulse makes this register notify on every 1 °C change of current temperature, the implementation logs only actual changes to the setting rather than every notification.
