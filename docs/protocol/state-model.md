...
- **Implementation status**: Implemented (read-only) in `components/volcano/volcano.cpp`. Verified against real hardware, including that panel-driven target changes are reported indistinguishably from commanded ones, matching [STATE-008](#state-008--statusflags-register-partial)'s finding for the status register.
