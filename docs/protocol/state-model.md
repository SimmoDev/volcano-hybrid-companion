...
- **Confidence**: Confirmed (polarity, and that the bit tracks the displayed unit on both target and current temperature as the panel gesture is performed); Unknown (whether the bit can be written to change units remotely — never attempted)
- **Implementation status**: Not implemented
