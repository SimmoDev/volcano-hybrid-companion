...
- **Interpretation**: **To detect the pump, test bit 12, not bit 13.** Bit 13 is set during a pump run but is also pulsed by every vibration alert, so a client testing bit 13 alone will report the pump running each time the device signals that it has reached temperature.
