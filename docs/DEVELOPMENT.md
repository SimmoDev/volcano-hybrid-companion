...
Phases 1–3 are complete and verified against real hardware. Phase 4 — release and distribution — is in progress: packaging the Dial firmware as a browser-flashable release, with device setup moved out of compile-time `secrets.yaml`, per [ADR-0013](decisions/ADR-0013-release-and-distribution.md). The repository is not yet tagged.
...
[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs on every pull request and on pushes to `main`: `esphome config`/`esphome compile` against both ESPHome configurations, each from its own placeholder `secrets.yaml` (per "Validating the component locally" above) — the Phase 1 dev-board scaffold under `examples/`, and the Dial firmware under `firmware/`, which connects the volcano component over BLE, joins WiFi (for its Home page's status icons, its own `web_server` page, and its Home Assistant `api` connection), and serves the full ADR-0011 local UI — `make test` for the Volcano component's host-side tests, a `clang-format --dry-run --Werror` check (see [`docs/CONVENTIONS.md`](CONVENTIONS.md#code-formatting-expectations) for the style and why it's pinned to an exact version), and `scripts/check_markdown_links.py` over every tracked `.md` file — the same check as the `check-markdown-links` pre-commit hook below, so a broken relative link is caught even without the hook installed. None of the four jobs need real hardware.
