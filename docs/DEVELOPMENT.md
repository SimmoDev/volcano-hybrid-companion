...
All four phases are complete and verified against real hardware, and the project is released as `v1.0.0` per [ADR-0013](decisions/ADR-0013-release-and-distribution.md): the Volcano's address, WiFi credentials and the Home Assistant API key have all moved from compile-time `secrets.yaml` to on-device runtime setup, the Dial firmware carries an over-the-air update path, and it can be installed from a browser at https://simmodev.github.io/volcano-hybrid-companion/ with no toolchain — verified end to end, including against the actual tagged build.

Phase 1: the BLE foundation and the Volcano component, hardware-verified on an ESP32-S3-WROOM-1-N16R8 development board, per [ADR-0004](decisions/ADR-0004-development-hardware-strategy.md). Phase 2: the M5Stack Dial's local UI, built per [ADR-0010](decisions/ADR-0010-dial-hardware-and-ui-framework.md)/[ADR-0011](decisions/ADR-0011-dial-ui-navigation-architecture.md), giving the Dial full control of the Volcano with no external dependency — the firmware the project will ship, under [`firmware/`](../firmware/README.md#m5stack-dialyaml). Phase 3: Home Assistant integration through the ESPHome `api` component, per [ADR-0012](decisions/ADR-0012-home-assistant-integration.md) — both ESPHome configurations carry an encrypted `api:` block, the Dial's Connections page shows Home Assistant connection status, and the standalone guarantee (the Dial keeps full control of the Volcano with Home Assistant stopped, the API key wrong, or WiFi absent, and does not reboot) is hardware-verified. Phase 4 added no `volcano` component or Dial UI behaviour — it was packaging, on-device provisioning, and an OTA path ([ADR-0013](decisions/ADR-0013-release-and-distribution.md)). BLE protocol discovery remains a living document ([`docs/protocol/`](protocol/README.md)).

What Phase 1 delivered, and what Phase 2 and 3 build on:
...
- **`firmware/`** — the device firmware the project will ship. One target: the M5Stack Dial (`firmware/m5stack-dial.yaml`, split into `firmware/dial/*.yaml` packages). See [`firmware/README.md`](../firmware/README.md).
- **`examples/`** — the ESP32-S3 dev-board scaffold config that exercises the `volcano` component in isolation; a compile check and BLE-only test surface, not device firmware. See [`examples/README.md`](../examples/README.md).
...
A fourth test covers `pairing.h` (`components/volcano/pairing.h`), which is not part of `VolcanoBleClient`: how the Dial decides which Volcano to control. It recognises one from its advertisement (the serial in the manufacturer data plus the name in the scan response, seen under one address), parses and formats the address text, and decides one/several/none once a scan window closes — the logic where a wrong match would adopt someone else's device, or another Storz & Bickel product, as the one the Dial drives.
...
[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs on every pull request and on pushes to `main`: `esphome config`/`esphome compile` against both ESPHome configurations — the Phase 1 dev-board scaffold under `examples/`, from its own placeholder `secrets.yaml` (per "Validating the component locally" above), and the Dial firmware under `firmware/`, which needs no `secrets.yaml` of its own since WiFi and the API key are both provisioned at runtime (ADR-0013) — connects the volcano component over BLE, joins WiFi (for its Home page's status icons, its own `web_server` page, and its Home Assistant `api` connection), and serves the full ADR-0011 local UI — `make test` for the Volcano component's host-side tests, a `clang-format --dry-run --Werror` check (see [`docs/CONVENTIONS.md`](CONVENTIONS.md#code-formatting-expectations) for the style and why it's pinned to an exact version), and `scripts/check_markdown_links.py` over every tracked `.md` file — the same check as the `check-markdown-links` pre-commit hook below, so a broken relative link is caught even without the hook installed. None of the four jobs need real hardware.

[`.github/workflows/release.yml`](../.github/workflows/release.yml) is separate: it builds the Dial's factory image against a pinned ESPHome version (unlike `ci.yml`'s own deliberately unpinned install — a release needs the same tagged commit to keep producing the same binary later) and, for a real `v*` tag, drafts a GitHub Release carrying it, after checking the tag matches `dial/hardware.yaml`'s own version string. `workflow_dispatch` runs the build alone, for trying the pipeline without a tag. See [ADR-0013](decisions/ADR-0013-release-and-distribution.md). `v1.0.0` is the one tag it's been run against for real.

[`.github/workflows/pages.yml`](../.github/workflows/pages.yml) deploys [`install/`](../install/README.md) — the browser install page — to GitHub Pages, on a push that touches it and whenever a release publishes (so the factory image it serves, fetched at deploy time rather than linked from GitHub Releases directly — see that page's own README for why — never lags behind). Live at https://simmodev.github.io/volcano-hybrid-companion/, confirmed flashing and provisioning a real device.
...
- **`readme-staleness-nudge`** (implemented in [`scripts/`](../scripts/)) — advisory only, never blocks. When a commit touches `components/volcano/`, `docs/decisions/`, `examples/`, `firmware/`, `install/` or a `.github/workflows/` file, it prints a reminder naming each of the root `README.md`, this document, [`examples/README.md`](../examples/README.md), [`firmware/README.md`](../firmware/README.md) and [`install/README.md`](../install/README.md) the commit does not also touch, since such a change can make an implementation-status, phase, capability, deployment-status, or entity-behaviour claim in any of them go stale — and touching one does nothing for a claim that has drifted in another.
