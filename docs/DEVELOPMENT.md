...
Phases 1–3 are complete and verified against real hardware. Phase 4 — release and distribution — is in progress, per [ADR-0013](decisions/ADR-0013-release-and-distribution.md): the Volcano's address, WiFi credentials and the Home Assistant API key have all moved from compile-time `secrets.yaml` to on-device runtime setup, and the Dial firmware carries an over-the-air update path — all verified against real hardware. Packaging the firmware itself as a browser-flashable release remains. The repository is not yet tagged.
