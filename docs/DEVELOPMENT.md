...
[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs on every push and pull request: `esphome config`/`esphome compile` against each example configuration, both sharing the same placeholder `secrets.yaml` (per "Validating the component locally" above) — the Phase 1 dev board's, and the M5Stack Dial's, which connects the volcano component over BLE, joins WiFi (for its Home page's status icons and its own `web_server` page), and serves the full ADR-0011 local UI, but has no `api:` yet (Phase 3's concern) — `make test` for `VolcanoDevice`'s host-side tests, and a `clang-format --dry-run --Werror` check (see [`docs/CONVENTIONS.md`](CONVENTIONS.md#code-formatting-expectations) for the style and why it's pinned to an exact version). None of the three jobs need real hardware.
