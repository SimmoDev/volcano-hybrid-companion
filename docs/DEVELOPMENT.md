...
- **`readme-staleness-nudge`** (implemented in [`scripts/`](../scripts/)) — advisory only, never blocks. When a commit touches `components/volcano/`, `docs/decisions/`, `examples/` or `firmware/`, it prints a reminder naming each of the root `README.md`, this document, [`examples/README.md`](../examples/README.md) and [`firmware/README.md`](../firmware/README.md) the commit does not also touch, since such a change can make an implementation-status, phase, capability, or entity-behaviour claim in any of them go stale — and touching one does nothing for a claim that has drifted in another.
