...
- **The Volcano component's internal architecture follows [ADR-0002](decisions/ADR-0002-volcano-component-architecture.md)**: BLE details stay inside the component, control interfaces depend on a hardware-independent interface, and the component owns the device state model. [ADR-0009](decisions/ADR-0009-volcano-abstraction-layer-interface.md) makes that boundary concrete — splitting the component into a BLE communication layer, an abstraction layer owning the state model, and the ESPHome integration consuming it. It is decided but not yet implemented: the component is currently one class, and the ESPHome entities still hold the only record of device state.
