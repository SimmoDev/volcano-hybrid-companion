...
- **Phase 3 exit checklist.** The Decision points above, consolidated as the gate for closing the milestone:
  ...
  - [x] The `firmware/` directory created and the first release, `v1.0.0`, cut — see the Resolved bullet below.
- **Resolved: `firmware/` created and the release versioned; Phase 3 closed.** The M5Stack Dial configuration moved from `examples/` to `firmware/m5stack-dial.yaml` (with its `dial/*.yaml` packages and DSEG font alongside); the ESP32-S3 dev-board scaffold stays under `examples/`, renamed `esp32-s3-devkit.yaml` since it is no longer a peer of a shipped config. The Dial firmware's `project: version` is set to `1.0.0` and the state-describing docs (root `README.md`, `docs/DEVELOPMENT.md`, `examples/README.md`, and the new `firmware/README.md`) updated to "Phase 3 complete". The commit carrying this is tagged `v1.0.0` in git — the project's first release. Every box above is now ticked and Phase 3 is closed.
