...
- The `firmware/` directory and the first versioned release, v1.0.0, still happen when Phase 3 completes, not at this ADR — unchanged from the existing project plan. *(Superseded in part: `firmware/` was created at Phase 3's close, but the versioned release `v1.0.0` is now the exit artefact of Phase 4 — see [ADR-0013](ADR-0013-release-and-distribution.md) and the Resolved bullets below.)*
...
- **Resolved: the verification the "Phase 3 is complete only when…" decision requires has been carried out.** On 2026-08-31, with the M5Stack Dial firmware against a real Home Assistant instance: the Volcano entities appeared in Home Assistant and were controllable from it; changes made at the Volcano's panel, on the Dial, and on the `web_server` page each reflected in Home Assistant, and Home Assistant's changes reflected back; the Dial's raw rotary-encoder and button inputs were confirmed absent from the entity list; and the gating check passed — with Home Assistant stopped, with the wrong encryption key configured, and with WiFi absent, the Dial kept full local control of the Volcano and did not reboot. The same session settled the Trade-offs section's free-memory question: with `api` running alongside BLE, LVGL and `web_server` on the RAM-constrained Dial, BLE stayed connected and every control kept working for the duration, with no allocation failure or instability. The `firmware/` directory still remained to be created before Phase 3 formally closed; the `v1.0.0` release named above has since moved to Phase 4 ([ADR-0013](ADR-0013-release-and-distribution.md)).
- **Both example configurations have been verified against a real Home Assistant instance.** The gating check above was performed on the Dial configuration. The dev-board configuration was then flashed to the ESP32-S3 devkit and exercised against Home Assistant separately: its entities were discovered and controllable, with no fault observed. The two configurations declare an identical `api` block and the dev board's `volcano` entity set is a strict subset of the Dial's (it has no Dial-firmware-version entity), so this confirmed rather than extended the Dial result.
- **Phase 3 exit checklist.** The Decision points above, consolidated as the gate for closing the milestone:
  - [x] `api` enabled on both example configurations, encrypted with a pre-shared key supplied from `secrets.yaml`.
  - [x] The `api` and `wifi` reboot timeouts both set to `0s`, so the device never reboots for want of an API client or a WiFi association.
  - [x] No Volcano-domain behaviour conditional on the API — the `volcano` component is unchanged by this milestone.
  - [x] Home Assistant's entity surface limited to the named `volcano` entities plus the Dial firmware-version diagnostic; the Dial's raw rotary-encoder and button inputs kept off it.
  - [x] The Dial's Connections page carries a read-only Home Assistant status row, recorded in [ADR-0011](ADR-0011-dial-ui-navigation-architecture.md)'s Notes.
  - [x] Releasing the BLE connection from Home Assistant left to the `ble_client` component's own enable/disable, with no dedicated entity.
  - [x] The integration and the standalone guarantee verified against a real Home Assistant instance on both configurations — see the two bullets above.
  - [x] The `firmware/` directory created. *(The versioned release `v1.0.0` was folded into this checklist item when it was written; it is now the exit artefact of Phase 4 — see [ADR-0013](ADR-0013-release-and-distribution.md) and the Resolved bullet below.)*
- **Resolved: `firmware/` created; Phase 3 closed on its own scope; the release moved to Phase 4.** The M5Stack Dial configuration moved from `examples/` to `firmware/m5stack-dial.yaml` (with its `dial/*.yaml` packages and DSEG font alongside); the ESP32-S3 dev-board scaffold stays under `examples/`, renamed `esp32-s3-devkit.yaml` since it is no longer a peer of the shipped config. The Dial firmware's `project: version` is set to `1.0.0` — the firmware's own version, not a release. Cutting the release itself — a `v1.0.0` git tag, a GitHub Release, and an install path a non-developer can use — turned out to be a milestone in its own right rather than a mechanical step, and is now **Phase 4 — Release and Distribution** ([ADR-0013](ADR-0013-release-and-distribution.md)). Phase 3 is closed on the Home Assistant integration this ADR governs, verified above; Phase 4's own tag and release are its concern, not this one's.
