...
- **Connect/disconnect are not yet on `VolcanoDevice`'s interface.** The Decision section above states that connect and disconnect are exposed as explicit commands on the component's interface. [ADR-0009](ADR-0009-volcano-abstraction-layer-interface.md), which finalised that interface, does not carry this requirement forward — `VolcanoDevice` and `VolcanoComponent` expose no connect/disconnect/release method. This is deliberate, not an oversight: per the Trade-offs above, Phase 1 is validation work rather than live use of the device as a controller, and release remains reachable through the underlying `ble_client` component's own enable/disable actions (ESPHome automations or the API) in the meantime. Revisit this when Phase 2 builds a user-facing control interface that needs to offer release directly, at which point either `VolcanoDevice`'s interface should gain it or this ADR should be amended to reflect that release stays outside the Volcano abstraction layer permanently.
- **Resolved: release stays outside `VolcanoDevice`'s interface permanently, per this note's second option above.** The Dial's local UI now offers a discoverable release/reconnect control, satisfying the Trade-offs section's requirement that a control interface built on this component make releasing the connection discoverable and explain why the device disappears when it does. It reaches the underlying connection directly rather than through an extension to `VolcanoDevice`'s interface — the same boundary [ADR-0011](ADR-0011-dial-ui-navigation-architecture.md)'s Notes already draw for Dial Brightness and Dial Sound: connection lifecycle control, like those, is local Dial-side plumbing, not a Volcano domain concept `VolcanoDevice` needs to own. `VolcanoDevice` and `VolcanoComponent` still expose no connect/disconnect/release method, and that is now this ADR's settled position rather than an open question.
