...
- The Context's and Decision's references to `api` being part of an "already built and hardware-verified" Phase 1 stack, and to `api` being "retained on the Dial firmware, unchanged from the dev-board example," were inaccurate when this ADR was written: no example configuration declared an `api:` block, and Home Assistant integration (Phase 3) had not started. [ADR-0012](ADR-0012-home-assistant-integration.md) is where `api` entered the project — it adds an encrypted `api:` block to both example configurations. `wifi` and `web_server` were retained on the Dial firmware from the outset, exactly as this ADR states; `api` arrived later, via ADR-0012. The Decision and Context text is left as written rather than rewritten, per the [ADR-0003](ADR-0003-esphome-as-firmware-framework.md) Notes precedent.
