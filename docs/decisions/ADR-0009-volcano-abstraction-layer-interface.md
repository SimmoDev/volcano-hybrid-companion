...
- **`CONNECTING` begins at service discovery completion, not at the raw physical link coming up.** The Decision section above describes `CONNECTING` as "link up or coming up; discovery, subscription and the initial read sweep have not all completed," which reads as starting the moment the physical link exists. The implementation instead stays `DISCONNECTED` until `VolcanoBleClient` observes `ESP_GATTC_SEARCH_CMPL_EVT` (service discovery complete) and only then reports `on_connecting()`; the brief window where the link is up but discovery has not yet started or completed is reported as `DISCONNECTED`. This is deliberate, not an oversight: both states already carry the same meaning to a consumer ("do not treat this as live"), and reporting a `CONNECTING` transition off the raw link-up event would require handling `ESP_GATTC_OPEN_EVT` inside `VolcanoBleClient`, whose exact semantics (timing relative to `client_`'s own state, whether `conn_id`/`gattc_if` are already valid) this project's own standard requires verifying against ESPHome/ESP-IDF source and re-testing against real hardware before landing — not yet done. Revisit if a future consumer needs to distinguish "the link exists but nothing has started yet" from "no link at all."
