...
- The concrete mechanisms — which provisioning method is used, where the install page is hosted, how the release build is wired — are implementation detail for the increments that follow, not fixed here, the same way [ADR-0012](ADR-0012-home-assistant-integration.md) left its `api` wiring to its increments.
- **How the Volcano address is discovered or entered**, resolving that part of the deferral above. The Dial finds the Volcano by scanning for its advertised identity rather than being given an address at build time: per [ADV-001](../protocol/gatt-services.md#adv-001--advertising-and-discovery) the device advertises manufacturer data under Storz & Bickel's company ID and the complete local name `S&B VOLCANO H`, and no service UUID — enough to recognise it — and the advertised address is static per unit, so it is stored on first pairing and used directly on every boot after that, from which point the Dial behaves exactly as the old compile-time address did. When discovery is not decisive: one match is adopted silently; several are offered for the user to choose between on the Pairing page, each identified by the serial its advertisement carries; none leaves the Pairing page reporting "not found" with a rescan, and the address can also be set or corrected from the `web_server` page or Home Assistant. A literal address-entry keypad on the Dial is deliberately not built — rotary-and-button hex entry is punitive, and WiFi provisioning already brings a browser within reach for the rare correction. A "forget and re-pair" action covers a replaced device. A Dial that has never been paired still boots and runs its local UI; only Volcano control waits on an address being known, consistent with the standalone-during-onboarding decision above.
- **How discovery recognises a Volcano.** The note above says the manufacturer data and the name are "enough to recognise it"; in practice both are required, and together. The company ID names Storz & Bickel, not the Volcano Hybrid, so the manufacturer data alone could adopt another of its products, and the name and the serial arrive separately, so a unit counts only once both have been seen from the same device. The search runs for a fixed window (10 seconds by default) before deciding, so that a single match is adopted without asking only after a second unit has had a chance to appear. The chosen address is stored on the device and used on every later boot. The paired address and a status line are also exposed through the web page and Home Assistant, and writing an empty address forgets the unit and searches again.
- The **Pairing page** that surfaces this is recorded in [ADR-0011](ADR-0011-dial-ui-navigation-architecture.md)'s Notes, as the Connections page was.
- **How WiFi and the API encryption key are provisioned, as built.** Both move to runtime the same way the Notes above anticipated, using mechanisms ESPHome itself already provides rather than anything bespoke. WiFi: the Dial ships with no network configured and offers to be provisioned over the same USB connection used to flash it, using the public Improv Wi-Fi protocol — the same one the eventual browser install page's flashing tool speaks natively, so nothing further is needed there once that page exists; a network agreed this way is stored on the device and read back on every later boot, the same as one that had been compiled in. The API encryption key: the Dial ships with none configured either, and accepts one bootstrap connection using a reserved, well-known placeholder key — enough to exchange a real one over an encrypted channel, not to grant lasting access — after which a real per-device key is agreed and used exclusively from then on; Home Assistant's own device-discovery flow already speaks this handshake unaided. Both close after a fixed window if neither is completed, rather than staying open indefinitely, closing again only on a power cycle; this affects only WiFi and Home Assistant reachability, never local BLE control, consistent with the standalone guarantee above. Verified end to end on real hardware, including the exclusivity of the agreed key once set and a device recovering the moment a real key is cleared.
- **How the OTA path is secured, as built.** `ota:` carries no password: it trusts the same network `web_server` already does, rather than reintroducing a compile-time secret for the one piece of the release image that would need one — the mechanism this platform offers has no runtime-provisioning story of its own, unlike WiFi and the API key above. `safe_mode:` needed no configuration beyond `ota:` enabling it: its defaults were kept, and the bootloader-level rollback ESP-IDF itself provides is what actually recovers from a bad update — a new image that never reports itself healthy is reverted to the one it replaced on the device's own next boot, before any of this project's own logic runs. Verified against real hardware: an update pushed and landing over the network while the Dial ran, and back again, with the paired Volcano's connection surviving both.
- **How the release build is wired, as built.** A tag-triggered workflow, pinned to an exact ESPHome version independent of the validation job's own deliberately-unpinned install -- the two now have different jobs to do, and only one of them wants to float. A pushed tag is checked against the firmware's own version string before anything is built from it, so the two can't quietly disagree; only a tag that matches reaches a compile, and only a compile that succeeds reaches a Release, created as a draft for a person to check and publish rather than going live unattended. Verified by simulating both a matching and a mismatched tag: the mismatch is caught before any build work starts, and the matching case builds correctly and produces the release's asset files, checked by hand rather than by pushing a real tag against the live repository. The very last step -- actually drafting the Release -- was reviewed rather than exercised live, for the same reason.
- **How the install page is wired, as built.** A static page (`install/`) using ESP Web Tools' install button, which flashes a factory image over WebSerial and, right after, offers to provision WiFi over the same connection via the same Improv mechanism `firmware/README.md`'s Onboarding section already describes -- one browser session covers both. The factory image is served from the same origin as the page itself, fetched from the latest release at deploy time, rather than linked directly to GitHub's own release asset storage: that storage's signed URLs carry no `Access-Control-Allow-Origin` header, and ESP Web Tools reads the image via `fetch()`, which enforces CORS unlike a plain download -- a same-origin request needs no such header. The Pages deploy workflow re-runs whenever a release publishes, so the hosted copy never falls behind; `release.yml`'s asset filenames stay fixed across releases rather than carrying the tag, so that download step's own pattern match keeps finding them. Deployed to GitHub Pages by its own workflow, kept separate from the release build. **Live** at https://simmodev.github.io/volcano-hybrid-companion/: a real device, flashed from the published page, walked through its own WiFi-provisioning prompt, both over the same browser session.
- **The standalone guarantee, re-verified on the actual `v1.0.0` release image.** Not the locally-built firmware earlier increments used -- the exact factory image downloaded from the tagged build's own draft Release, written with `esptool` the same way the install page would, onto a fully erased device (confirmed blank by reading its own flash back before writing anything, not assumed). It booted with no stored Volcano, no WiFi, no API key -- and read the Volcano's full state over BLE regardless, before any of the three resolved. WiFi provisioning, pairing and the API key bootstrap were not repeated here; each was already verified on its own increment, and this check's job was only to confirm none of them gates the others on a real release build.
...
- **Phase 4 exit checklist.** The Decision points above, consolidated as the gate for cutting the release:
  ...
  - [x] The standalone guarantee re-verified on the release image: a flashed-but-unprovisioned device boots, and local control never blocks on onboarding being finished.
  - [x] `firmware/README.md` documents the browser install path and the supported-browser matrix, with the from-source CLI path kept alongside it.
  - [x] The git tag `v1.0.0`, a GitHub Release with the image attached, and the published install page — created together.
- **Resolved: Phase 4 is complete.** Every item above is done and verified against real hardware, the last of them by a full install through the published page itself — a freshly erased Dial, flashed from the browser, walked through its own WiFi-provisioning prompt, connected and paired. `v1.0.0` is tagged, released, and installable by anyone with Chrome or Edge and a USB-C cable, per ADR-0001's original goal for this project.
