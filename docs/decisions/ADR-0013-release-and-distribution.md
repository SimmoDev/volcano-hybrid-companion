...
- **How the install page is wired, as built.** A static page (`install/`) using ESP Web Tools' install button, which flashes a factory image over WebSerial and, right after, offers to provision WiFi over the same connection via the same Improv mechanism `firmware/README.md`'s Onboarding section already describes -- one browser session covers both. Deployed to GitHub Pages by its own workflow, kept separate from the release build. **Live** at https://simmodev.github.io/volcano-hybrid-companion/, confirmed serving the real page and manifest, and, once `v1.0.0` was published, confirmed flashing a real device.
- **Correction: the factory image is not linked from GitHub Releases after all.** The Note above originally had `manifest.json` pointing at the release build's factory image by GitHub's own stable "latest" alias, which looked right -- a plain download, or pasting the URL into a browser, both worked -- but failed from the install button itself with "Failed to fetch". ESP Web Tools reads the image via `fetch()`, which enforces CORS unlike a plain download; GitHub's release asset storage redirects to a signed URL carrying no `Access-Control-Allow-Origin` header at all, confirmed by reading its response headers directly, so a browser refuses to let cross-origin JavaScript read it even though every other way of fetching it works. The Pages deploy workflow now downloads the latest release's factory image itself (an ordinary server-side download, not subject to CORS) and serves it from the same origin as the page, re-running whenever a release publishes so it never falls behind. `release.yml`'s asset filenames still stay fixed across releases rather than carrying the tag -- that requirement didn't go away, it just now serves this download step's own pattern match rather than a URL in `manifest.json`.
