...
- **How the install page is wired, as built.** A static page (`install/`) using ESP Web Tools' install button, which flashes a factory image over WebSerial and, right after, offers to provision WiFi over the same connection via the same Improv mechanism `firmware/README.md`'s Onboarding section already describes -- one browser session covers both. It points at the release build's factory image by GitHub's own stable "latest" alias rather than a specific version, so it keeps working across future releases unattended, which is also why `release.yml`'s asset filenames stay fixed across releases rather than carrying the tag (a correction to the previous Note: they did carry it, until this one). Deployed to GitHub Pages by its own workflow, kept separate from the release build. **Live** at https://simmodev.github.io/volcano-hybrid-companion/, confirmed serving the real page and manifest -- but the install button itself does not yet work, since `manifest.json` points at a release that does not exist. That remains for the increment that actually cuts one.
