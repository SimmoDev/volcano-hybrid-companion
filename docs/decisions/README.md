...
- **[ADR-0012](ADR-0012-home-assistant-integration.md) — Home Assistant Integration.** An encrypted `api` on both ESPHome configurations, the `api` and `wifi` reboot timeouts disabled so the device never reboots for want of a network, and a read-only Home Assistant status row on the Dial's Connections page.
- **[ADR-0013](ADR-0013-release-and-distribution.md) — Release and Distribution.** A fourth phase: the Dial firmware distributed as a prebuilt binary flashed from a web browser, with WiFi, the Volcano address and a per-device API key supplied at runtime rather than compile time, an over-the-air update path, and `v1.0.0` tagged as the phase's exit artefact.
