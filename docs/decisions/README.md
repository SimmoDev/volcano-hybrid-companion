...
- **[ADR-0012](ADR-0012-home-assistant-integration.md) — Home Assistant Integration.** An encrypted `api` on both example configurations, the reboot-on-no-client timeout disabled, and a read-only Home Assistant status row on the Dial's Connections page.
