...
- Maintaining the Volcano device state model — current temperature, target temperature, heater state, pump state, and any other observable device state — as the single, authoritative copy of that state.
- Providing a hardware-independent interface for control interfaces, expressed purely in Volcano domain terms (e.g. "set target temperature," "current state changed") with no BLE types, service UUIDs, or characteristic handles in its public surface.
...
**What control interfaces must not do**
...
- BLE protocol logic exists in exactly one place, so protocol fixes, quirks, and refinements apply to every control interface simultaneously.
...
- A new control interface can be added by writing a consumer of the existing interface, without touching or duplicating BLE logic.

**Trade-offs**

- Every capability a control interface needs must first be exposed by the Volcano component's interface; a UI feature that needs a device value the component doesn't yet expose requires extending the component first, not just the UI.
...
Device state lives in Home Assistant, with the ESP32 relaying raw values to it and other control interfaces (the Dial, direct API) reading state back through Home Assistant. Rejected because it makes Home Assistant load-bearing for basic device state, directly contradicting ADR-0001's requirement that the device keep working when Home Assistant is unreachable, and because Home Assistant is one control interface among three, not a privileged one.
