...
- The Connections page gained a third status line, **Home Assistant**, per [ADR-0012](ADR-0012-home-assistant-integration.md). Unlike the BLE and WiFi rows it is indicator only — no switch (nothing about the ESPHome `api` connection needs on-device control) and no Home-page icon (ADR-0012 scopes the indicator to this page). It reports whether an `api` client is currently connected. Adding it took the page from six stacked groups to eight, past the layout budget the Settings page established; the BLE status text was shortened to one short line to compensate, and the eight-group fit was verified on real Dial hardware.
