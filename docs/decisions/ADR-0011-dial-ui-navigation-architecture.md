...
- The Decision's Home page section names "a strikethrough variant for disconnected" for the BLE/WiFi status icons. LVGL's built-in symbol set has no strikethrough glyph, so the implementation signals disconnected with colour alone (steady red). This falls under the icon-artwork allowance above; it is called out here only because the Decision text named a specific treatment.
- The Decision's "Information page" section describes one page; the implementation splits it into two, **About** and **Diagnostics**, each its own Navigation Menu entry — a single "Label: value" line combining both read too plain on real hardware, per user feedback. Left as a difference from the Decision text rather than rewritten, per the [ADR-0003](ADR-0003-esphome-as-firmware-framework.md) Notes precedent; the page count elsewhere in this ADR's Page model should be read as nine pages, not eight.
- A tenth page, **Connections**, was added beyond this ADR's own page set: BLE and WiFi connection status, each with a switch to release/reconnect it. It closes the gap [ADR-0007](ADR-0007-ble-connection-lifecycle.md) left open — see that ADR's own Notes, which this addition resolves. It sits second in the Navigation Menu, immediately after Home, ahead of Settings: connectivity is a more fundamental concern than the Volcano behaviour settings Settings governs, and is often the reason a user opens the menu at all. Dial Brightness's and Dial Sound's exemption from `VolcanoDevice`, noted above, extends to this page's controls too — connection lifecycle is local Dial-side plumbing, not a Volcano domain concept.
- The "connecting" state the Home and Connections pages show is derived from the underlying `ble_client` link coming up while the component's `connected` entity is still off, not from a dedicated connection-state entity — [ADR-0009](ADR-0009-volcano-abstraction-layer-interface.md)'s Notes record why its three-value `ConnectionState` is not promoted to the entity surface for this.
- The Connections page gained a third status line, **Home Assistant**, per [ADR-0012](ADR-0012-home-assistant-integration.md). Unlike the BLE and WiFi rows it is indicator only — no switch (nothing about the ESPHome `api` connection needs on-device control) and no Home-page icon (ADR-0012 scopes the indicator to this page). It reports whether an `api` client is currently connected. The page carries eight stacked groups, past the layout budget the Settings page set, so the BLE status text is held to one short line; all eight clear the round display without clipping, verified on real Dial hardware.
- An eleventh page, **Pairing**, was added beyond both this ADR's page set and the Connections page above. It is where the user establishes *which* Volcano the Dial controls, as distinct from Connections, which manages links to an already-known device: it shows discovery status and offers a rescan, a choice between multiple discovered units, and a "forget" that clears the current one so another can be paired. It is a separate page rather than more rows on Connections because that is a separate concern — and a mostly one-time one — and because Connections is already at the layout budget the previous bullet records. It sits immediately after Connections in the Navigation Menu, the two being adjacent connectivity concerns; on first run, before any Volcano is known, the Dial opens Pairing directly rather than leaving the user to find it. As with Connections, choosing and storing the target device is local Dial-side plumbing, not a Volcano domain concept, so it stays outside `VolcanoDevice` by the same boundary. The discovery mechanism this page is the surface for, and the runtime target address it depends on, are [ADR-0013](ADR-0013-release-and-distribution.md)'s.
